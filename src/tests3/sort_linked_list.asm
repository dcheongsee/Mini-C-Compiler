.data
.align 2
global_val:
.space 4
.align 2
global_next:
.space 4
.align 2
global_c:
.space 1
.align 2
global_size:
.space 4
.align 2
global_s:
.space 4
.align 2
global_i:
.space 4
# Allocated labels for virtual registers
.align 2
label_54_v13:
.space 4
.align 2
label_45_v11:
.space 4
.align 2
label_50_v14:
.space 4
.align 2
label_33_v5:
.space 4
.align 2
label_35_v6:
.space 4
.align 2
label_27_v2:
.space 4
.align 2
label_31_v4:
.space 4
.align 2
label_44_v8:
.space 4
.align 2
label_47_v12:
.space 4
.align 2
label_23_v0:
.space 4
.align 2
label_52_v15:
.space 4
.align 2
label_40_v9:
.space 4
.align 2
label_30_v3:
.space 4
.align 2
label_37_v7:
.space 4
.align 2
label_55_v16:
.space 4
.align 2
label_42_v10:
.space 4
.align 2
label_25_v1:
.space 4
# Allocated labels for virtual registers
.align 2
label_164_v68:
.space 4
.align 2
label_168_v70:
.space 4
.align 2
label_117_v44:
.space 4
.align 2
label_64_v18:
.space 4
.align 2
label_89_v32:
.space 4
.align 2
label_145_v58:
.space 4
.align 2
label_77_v26:
.space 4
.align 2
label_95_v30:
.space 4
.align 2
label_73_v20:
.space 4
.align 2
label_105_v39:
.space 4
.align 2
label_172_v72:
.space 4
.align 2
label_121_v47:
.space 4
.align 2
label_127_v49:
.space 4
.align 2
label_161_v66:
.space 4
.align 2
label_132_v52:
.space 4
.align 2
label_88_v31:
.space 4
.align 2
label_101_v37:
.space 4
.align 2
label_120_v46:
.space 4
.align 2
label_157_v64:
.space 4
.align 2
label_92_v34:
.space 4
.align 2
label_123_v45:
.space 4
.align 2
label_104_v38:
.space 4
.align 2
label_70_v24:
.space 4
.align 2
label_136_v54:
.space 4
.align 2
label_142_v57:
.space 4
.align 2
label_138_v55:
.space 4
.align 2
label_71_v23:
.space 4
.align 2
label_79_v27:
.space 4
.align 2
label_143_v51:
.space 4
.align 2
label_153_v62:
.space 4
.align 2
label_158_v65:
.space 4
.align 2
label_140_v56:
.space 4
.align 2
label_76_v25:
.space 4
.align 2
label_111_v42:
.space 4
.align 2
label_162_v67:
.space 4
.align 2
label_125_v48:
.space 4
.align 2
label_171_v71:
.space 4
.align 2
label_66_v21:
.space 4
.align 2
label_99_v36:
.space 4
.align 2
label_147_v59:
.space 4
.align 2
label_107_v40:
.space 4
.align 2
label_86_v28:
.space 4
.align 2
label_93_v33:
.space 4
.align 2
label_149_v60:
.space 4
.align 2
label_98_v35:
.space 4
.align 2
label_155_v63:
.space 4
.align 2
label_85_v29:
.space 4
.align 2
label_63_v19:
.space 4
.align 2
label_118_v43:
.space 4
.align 2
label_67_v22:
.space 4
.align 2
label_129_v50:
.space 4
.align 2
label_134_v53:
.space 4
.align 2
label_151_v61:
.space 4
.align 2
label_166_v69:
.space 4
.align 2
label_57_v17:
.space 4
.align 2
label_109_v41:
.space 4
# Allocated labels for virtual registers
.align 2
label_217_v95:
.space 4
.align 2
label_242_v107:
.space 4
.align 2
label_244_v108:
.space 4
.align 2
label_239_v105:
.space 4
.align 2
label_190_v81:
.space 4
.align 2
label_237_v104:
.space 4
.align 2
label_178_v75:
.space 4
.align 2
label_199_v86:
.space 4
.align 2
label_183_v76:
.space 4
.align 2
label_189_v79:
.space 4
.align 2
label_184_v78:
.space 4
.align 2
label_208_v90:
.space 4
.align 2
label_196_v84:
.space 4
.align 2
label_212_v92:
.space 4
.align 2
label_177_v73:
.space 4
.align 2
label_235_v103:
.space 4
.align 2
label_225_v98:
.space 4
.align 2
label_246_v109:
.space 4
.align 2
label_207_v88:
.space 4
.align 2
label_193_v83:
.space 4
.align 2
label_202_v87:
.space 4
.align 2
label_219_v96:
.space 4
.align 2
label_214_v93:
.space 4
.align 2
label_233_v102:
.space 4
.align 2
label_205_v89:
.space 4
.align 2
label_249_v110:
.space 4
.align 2
label_181_v77:
.space 4
.align 2
label_250_v111:
.space 4
.align 2
label_195_v82:
.space 4
.align 2
label_226_v97:
.space 4
.align 2
label_231_v99:
.space 4
.align 2
label_216_v94:
.space 4
.align 2
label_175_v74:
.space 4
.align 2
label_211_v91:
.space 4
.align 2
label_229_v101:
.space 4
.align 2
label_201_v85:
.space 4
.align 2
label_187_v80:
.space 4
.align 2
label_240_v106:
.space 4
.align 2
label_228_v100:
.space 4

.text
swap_nodes:
# Original instruction: addiu $sp,$sp,-8
addiu $sp,$sp,-8
# Original instruction: sw $fp,4($sp)
sw $fp,4($sp)
# Original instruction: sw $ra,0($sp)
sw $ra,0($sp)
# Original instruction: add $fp,$sp,$zero
add $fp,$sp,$zero
# Original instruction: addiu $sp,$sp,-12
addiu $sp,$sp,-12
# Original instruction: addiu v0,$fp,8
addiu $t5,$fp,8
la $t0,label_23_v0
sw $t5,0($t0)
# Original instruction: lw v1,0(v0)
la $t5,label_23_v0
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_25_v1
sw $t4,0($t0)
# Original instruction: addiu v2,v1,0
la $t5,label_25_v1
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_27_v2
sw $t4,0($t0)
# Original instruction: lw v3,0(v2)
la $t5,label_27_v2
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_30_v3
sw $t4,0($t0)
# Original instruction: addiu v4,$fp,-12
addiu $t5,$fp,-12
la $t0,label_31_v4
sw $t5,0($t0)
# Original instruction: sw v3,0(v4)
la $t5,label_30_v3
lw $t5,0($t5)
la $t4,label_31_v4
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v5,$fp,12
addiu $t5,$fp,12
la $t0,label_33_v5
sw $t5,0($t0)
# Original instruction: lw v6,0(v5)
la $t5,label_33_v5
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_35_v6
sw $t4,0($t0)
# Original instruction: addiu v7,v6,0
la $t5,label_35_v6
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_37_v7
sw $t4,0($t0)
# Original instruction: lw v8,0(v7)
la $t5,label_37_v7
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_44_v8
sw $t4,0($t0)
# Original instruction: addiu v9,$fp,8
addiu $t5,$fp,8
la $t0,label_40_v9
sw $t5,0($t0)
# Original instruction: lw v10,0(v9)
la $t5,label_40_v9
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_42_v10
sw $t4,0($t0)
# Original instruction: addiu v11,v10,0
la $t5,label_42_v10
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_45_v11
sw $t4,0($t0)
# Original instruction: sw v8,0(v11)
la $t5,label_44_v8
lw $t5,0($t5)
la $t4,label_45_v11
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v12,$fp,-12
addiu $t5,$fp,-12
la $t0,label_47_v12
sw $t5,0($t0)
# Original instruction: lw v13,0(v12)
la $t5,label_47_v12
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_54_v13
sw $t4,0($t0)
# Original instruction: addiu v14,$fp,12
addiu $t5,$fp,12
la $t0,label_50_v14
sw $t5,0($t0)
# Original instruction: lw v15,0(v14)
la $t5,label_50_v14
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_52_v15
sw $t4,0($t0)
# Original instruction: addiu v16,v15,0
la $t5,label_52_v15
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_55_v16
sw $t4,0($t0)
# Original instruction: sw v13,0(v16)
la $t5,label_54_v13
lw $t5,0($t5)
la $t4,label_55_v16
lw $t4,0($t4)
sw $t5,0($t4)
label_7_return_swap_nodes:
# Original instruction: addiu $sp,$sp,12
addiu $sp,$sp,12
# Original instruction: lw $ra,0($sp)
lw $ra,0($sp)
# Original instruction: lw $fp,4($sp)
lw $fp,4($sp)
# Original instruction: addiu $sp,$sp,8
addiu $sp,$sp,8
# Original instruction: jr $ra
jr $ra

.text
bubble_sort:
# Original instruction: addiu $sp,$sp,-8
addiu $sp,$sp,-8
# Original instruction: sw $fp,4($sp)
sw $fp,4($sp)
# Original instruction: sw $ra,0($sp)
sw $ra,0($sp)
# Original instruction: add $fp,$sp,$zero
add $fp,$sp,$zero
# Original instruction: addiu $sp,$sp,-16
addiu $sp,$sp,-16
# Original instruction: addiu v17,$fp,8
addiu $t5,$fp,8
la $t0,label_57_v17
sw $t5,0($t0)
# Original instruction: lw v18,0(v17)
la $t5,label_57_v17
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_64_v18
sw $t4,0($t0)
# Original instruction: li v19,0
li $t5,0
la $t0,label_63_v19
sw $t5,0($t0)
# Original instruction: slt v21,v18,v19
la $t5,label_64_v18
lw $t5,0($t5)
la $t4,label_63_v19
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_66_v21
sw $t3,0($t0)
# Original instruction: slt v22,v19,v18
la $t5,label_63_v19
lw $t5,0($t5)
la $t4,label_64_v18
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_67_v22
sw $t3,0($t0)
# Original instruction: or v23,v21,v22
la $t5,label_66_v21
lw $t5,0($t5)
la $t4,label_67_v22
lw $t4,0($t4)
or $t3,$t5,$t4
la $t0,label_71_v23
sw $t3,0($t0)
# Original instruction: li v24,1
li $t5,1
la $t0,label_70_v24
sw $t5,0($t0)
# Original instruction: sub v20,v24,v23
la $t5,label_70_v24
lw $t5,0($t5)
la $t4,label_71_v23
lw $t4,0($t4)
sub $t3,$t5,$t4
la $t0,label_73_v20
sw $t3,0($t0)
# Original instruction: beqz v20,label_10_if_else
la $t5,label_73_v20
lw $t5,0($t5)
beqz $t5,label_10_if_else
# Original instruction: j label_9_return_bubble_sort
j label_9_return_bubble_sort
# Original instruction: j label_11_if_end
j label_11_if_end
label_10_if_else:
label_11_if_end:
# Original instruction: li v25,1
li $t5,1
la $t0,label_76_v25
sw $t5,0($t0)
# Original instruction: addiu v26,$fp,-16
addiu $t5,$fp,-16
la $t0,label_77_v26
sw $t5,0($t0)
# Original instruction: sw v25,0(v26)
la $t5,label_76_v25
lw $t5,0($t5)
la $t4,label_77_v26
lw $t4,0($t4)
sw $t5,0($t4)
label_12_while_start:
# Original instruction: addiu v27,$fp,-16
addiu $t5,$fp,-16
la $t0,label_79_v27
sw $t5,0($t0)
# Original instruction: lw v28,0(v27)
la $t5,label_79_v27
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_86_v28
sw $t4,0($t0)
# Original instruction: li v29,1
li $t5,1
la $t0,label_85_v29
sw $t5,0($t0)
# Original instruction: slt v31,v28,v29
la $t5,label_86_v28
lw $t5,0($t5)
la $t4,label_85_v29
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_88_v31
sw $t3,0($t0)
# Original instruction: slt v32,v29,v28
la $t5,label_85_v29
lw $t5,0($t5)
la $t4,label_86_v28
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_89_v32
sw $t3,0($t0)
# Original instruction: or v33,v31,v32
la $t5,label_88_v31
lw $t5,0($t5)
la $t4,label_89_v32
lw $t4,0($t4)
or $t3,$t5,$t4
la $t0,label_93_v33
sw $t3,0($t0)
# Original instruction: li v34,1
li $t5,1
la $t0,label_92_v34
sw $t5,0($t0)
# Original instruction: sub v30,v34,v33
la $t5,label_92_v34
lw $t5,0($t5)
la $t4,label_93_v33
lw $t4,0($t4)
sub $t3,$t5,$t4
la $t0,label_95_v30
sw $t3,0($t0)
# Original instruction: beqz v30,label_13_while_exit
la $t5,label_95_v30
lw $t5,0($t5)
beqz $t5,label_13_while_exit
# Original instruction: li v35,0
li $t5,0
la $t0,label_98_v35
sw $t5,0($t0)
# Original instruction: addiu v36,$fp,-16
addiu $t5,$fp,-16
la $t0,label_99_v36
sw $t5,0($t0)
# Original instruction: sw v35,0(v36)
la $t5,label_98_v35
lw $t5,0($t5)
la $t4,label_99_v36
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v37,$fp,8
addiu $t5,$fp,8
la $t0,label_101_v37
sw $t5,0($t0)
# Original instruction: lw v38,0(v37)
la $t5,label_101_v37
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_104_v38
sw $t4,0($t0)
# Original instruction: addiu v39,$fp,-12
addiu $t5,$fp,-12
la $t0,label_105_v39
sw $t5,0($t0)
# Original instruction: sw v38,0(v39)
la $t5,label_104_v38
lw $t5,0($t5)
la $t4,label_105_v39
lw $t4,0($t4)
sw $t5,0($t4)
label_14_while_start:
# Original instruction: addiu v40,$fp,-12
addiu $t5,$fp,-12
la $t0,label_107_v40
sw $t5,0($t0)
# Original instruction: lw v41,0(v40)
la $t5,label_107_v40
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_109_v41
sw $t4,0($t0)
# Original instruction: addiu v42,v41,4
la $t5,label_109_v41
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_111_v42
sw $t4,0($t0)
# Original instruction: lw v43,0(v42)
la $t5,label_111_v42
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_118_v43
sw $t4,0($t0)
# Original instruction: li v44,0
li $t5,0
la $t0,label_117_v44
sw $t5,0($t0)
# Original instruction: slt v46,v43,v44
la $t5,label_118_v43
lw $t5,0($t5)
la $t4,label_117_v44
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_120_v46
sw $t3,0($t0)
# Original instruction: slt v47,v44,v43
la $t5,label_117_v44
lw $t5,0($t5)
la $t4,label_118_v43
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_121_v47
sw $t3,0($t0)
# Original instruction: or v45,v46,v47
la $t5,label_120_v46
lw $t5,0($t5)
la $t4,label_121_v47
lw $t4,0($t4)
or $t3,$t5,$t4
la $t0,label_123_v45
sw $t3,0($t0)
# Original instruction: beqz v45,label_15_while_exit
la $t5,label_123_v45
lw $t5,0($t5)
beqz $t5,label_15_while_exit
# Original instruction: addiu v48,$fp,-12
addiu $t5,$fp,-12
la $t0,label_125_v48
sw $t5,0($t0)
# Original instruction: lw v49,0(v48)
la $t5,label_125_v48
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_127_v49
sw $t4,0($t0)
# Original instruction: addiu v50,v49,0
la $t5,label_127_v49
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_129_v50
sw $t4,0($t0)
# Original instruction: lw v51,0(v50)
la $t5,label_129_v50
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_143_v51
sw $t4,0($t0)
# Original instruction: addiu v52,$fp,-12
addiu $t5,$fp,-12
la $t0,label_132_v52
sw $t5,0($t0)
# Original instruction: lw v53,0(v52)
la $t5,label_132_v52
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_134_v53
sw $t4,0($t0)
# Original instruction: addiu v54,v53,4
la $t5,label_134_v53
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_136_v54
sw $t4,0($t0)
# Original instruction: lw v55,0(v54)
la $t5,label_136_v54
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_138_v55
sw $t4,0($t0)
# Original instruction: addiu v56,v55,0
la $t5,label_138_v55
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_140_v56
sw $t4,0($t0)
# Original instruction: lw v57,0(v56)
la $t5,label_140_v56
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_142_v57
sw $t4,0($t0)
# Original instruction: slt v58,v57,v51
la $t5,label_142_v57
lw $t5,0($t5)
la $t4,label_143_v51
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_145_v58
sw $t3,0($t0)
# Original instruction: beqz v58,label_16_if_else
la $t5,label_145_v58
lw $t5,0($t5)
beqz $t5,label_16_if_else
# Original instruction: addiu v59,$fp,-12
addiu $t5,$fp,-12
la $t0,label_147_v59
sw $t5,0($t0)
# Original instruction: lw v60,0(v59)
la $t5,label_147_v59
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_149_v60
sw $t4,0($t0)
# Original instruction: addiu v61,v60,4
la $t5,label_149_v60
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_151_v61
sw $t4,0($t0)
# Original instruction: lw v62,0(v61)
la $t5,label_151_v61
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_153_v62
sw $t4,0($t0)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw v62,0($sp)
la $t5,label_153_v62
lw $t5,0($t5)
sw $t5,0($sp)
# Original instruction: addiu v63,$fp,-12
addiu $t5,$fp,-12
la $t0,label_155_v63
sw $t5,0($t0)
# Original instruction: lw v64,0(v63)
la $t5,label_155_v63
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_157_v64
sw $t4,0($t0)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw v64,0($sp)
la $t5,label_157_v64
lw $t5,0($t5)
sw $t5,0($sp)
# Original instruction: jal swap_nodes
jal swap_nodes
# Original instruction: addiu $sp,$sp,8
addiu $sp,$sp,8
# Original instruction: add v65,$v0,$zero
add $t5,$v0,$zero
la $t0,label_158_v65
sw $t5,0($t0)
# Original instruction: li v66,1
li $t5,1
la $t0,label_161_v66
sw $t5,0($t0)
# Original instruction: addiu v67,$fp,-16
addiu $t5,$fp,-16
la $t0,label_162_v67
sw $t5,0($t0)
# Original instruction: sw v66,0(v67)
la $t5,label_161_v66
lw $t5,0($t5)
la $t4,label_162_v67
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: j label_17_if_end
j label_17_if_end
label_16_if_else:
label_17_if_end:
# Original instruction: addiu v68,$fp,-12
addiu $t5,$fp,-12
la $t0,label_164_v68
sw $t5,0($t0)
# Original instruction: lw v69,0(v68)
la $t5,label_164_v68
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_166_v69
sw $t4,0($t0)
# Original instruction: addiu v70,v69,4
la $t5,label_166_v69
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_168_v70
sw $t4,0($t0)
# Original instruction: lw v71,0(v70)
la $t5,label_168_v70
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_171_v71
sw $t4,0($t0)
# Original instruction: addiu v72,$fp,-12
addiu $t5,$fp,-12
la $t0,label_172_v72
sw $t5,0($t0)
# Original instruction: sw v71,0(v72)
la $t5,label_171_v71
lw $t5,0($t5)
la $t4,label_172_v72
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: j label_14_while_start
j label_14_while_start
label_15_while_exit:
# Original instruction: j label_12_while_start
j label_12_while_start
label_13_while_exit:
label_9_return_bubble_sort:
# Original instruction: addiu $sp,$sp,16
addiu $sp,$sp,16
# Original instruction: lw $ra,0($sp)
lw $ra,0($sp)
# Original instruction: lw $fp,4($sp)
lw $fp,4($sp)
# Original instruction: addiu $sp,$sp,8
addiu $sp,$sp,8
# Original instruction: jr $ra
jr $ra

.text
.globl main
main:
# Original instruction: addiu $sp,$sp,-8
addiu $sp,$sp,-8
# Original instruction: sw $fp,4($sp)
sw $fp,4($sp)
# Original instruction: sw $ra,0($sp)
sw $ra,0($sp)
# Original instruction: add $fp,$sp,$zero
add $fp,$sp,$zero
# Original instruction: addiu $sp,$sp,-36
addiu $sp,$sp,-36
# Original instruction: li v73,30
li $t5,30
la $t0,label_177_v73
sw $t5,0($t0)
# Original instruction: addiu v74,$fp,-16
addiu $t5,$fp,-16
la $t0,label_175_v74
sw $t5,0($t0)
# Original instruction: addiu v75,v74,0
la $t5,label_175_v74
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_178_v75
sw $t4,0($t0)
# Original instruction: sw v73,0(v75)
la $t5,label_177_v73
lw $t5,0($t5)
la $t4,label_178_v75
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: li v76,10
li $t5,10
la $t0,label_183_v76
sw $t5,0($t0)
# Original instruction: addiu v77,$fp,-24
addiu $t5,$fp,-24
la $t0,label_181_v77
sw $t5,0($t0)
# Original instruction: addiu v78,v77,0
la $t5,label_181_v77
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_184_v78
sw $t4,0($t0)
# Original instruction: sw v76,0(v78)
la $t5,label_183_v76
lw $t5,0($t5)
la $t4,label_184_v78
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: li v79,20
li $t5,20
la $t0,label_189_v79
sw $t5,0($t0)
# Original instruction: addiu v80,$fp,-32
addiu $t5,$fp,-32
la $t0,label_187_v80
sw $t5,0($t0)
# Original instruction: addiu v81,v80,0
la $t5,label_187_v80
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_190_v81
sw $t4,0($t0)
# Original instruction: sw v79,0(v81)
la $t5,label_189_v79
lw $t5,0($t5)
la $t4,label_190_v81
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v82,$fp,-24
addiu $t5,$fp,-24
la $t0,label_195_v82
sw $t5,0($t0)
# Original instruction: addiu v83,$fp,-16
addiu $t5,$fp,-16
la $t0,label_193_v83
sw $t5,0($t0)
# Original instruction: addiu v84,v83,4
la $t5,label_193_v83
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_196_v84
sw $t4,0($t0)
# Original instruction: sw v82,0(v84)
la $t5,label_195_v82
lw $t5,0($t5)
la $t4,label_196_v84
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v85,$fp,-32
addiu $t5,$fp,-32
la $t0,label_201_v85
sw $t5,0($t0)
# Original instruction: addiu v86,$fp,-24
addiu $t5,$fp,-24
la $t0,label_199_v86
sw $t5,0($t0)
# Original instruction: addiu v87,v86,4
la $t5,label_199_v86
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_202_v87
sw $t4,0($t0)
# Original instruction: sw v85,0(v87)
la $t5,label_201_v85
lw $t5,0($t5)
la $t4,label_202_v87
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: li v88,0
li $t5,0
la $t0,label_207_v88
sw $t5,0($t0)
# Original instruction: addiu v89,$fp,-32
addiu $t5,$fp,-32
la $t0,label_205_v89
sw $t5,0($t0)
# Original instruction: addiu v90,v89,4
la $t5,label_205_v89
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_208_v90
sw $t4,0($t0)
# Original instruction: sw v88,0(v90)
la $t5,label_207_v88
lw $t5,0($t5)
la $t4,label_208_v90
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v91,$fp,-16
addiu $t5,$fp,-16
la $t0,label_211_v91
sw $t5,0($t0)
# Original instruction: addiu v92,$fp,-36
addiu $t5,$fp,-36
la $t0,label_212_v92
sw $t5,0($t0)
# Original instruction: sw v91,0(v92)
la $t5,label_211_v91
lw $t5,0($t5)
la $t4,label_212_v92
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: addiu v93,$fp,-36
addiu $t5,$fp,-36
la $t0,label_214_v93
sw $t5,0($t0)
# Original instruction: lw v94,0(v93)
la $t5,label_214_v93
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_216_v94
sw $t4,0($t0)
# Original instruction: addiu $sp,$sp,-4
addiu $sp,$sp,-4
# Original instruction: sw v94,0($sp)
la $t5,label_216_v94
lw $t5,0($t5)
sw $t5,0($sp)
# Original instruction: jal bubble_sort
jal bubble_sort
# Original instruction: addiu $sp,$sp,4
addiu $sp,$sp,4
# Original instruction: add v95,$v0,$zero
add $t5,$v0,$zero
la $t0,label_217_v95
sw $t5,0($t0)
label_20_while_start:
# Original instruction: addiu v96,$fp,-36
addiu $t5,$fp,-36
la $t0,label_219_v96
sw $t5,0($t0)
# Original instruction: lw v97,0(v96)
la $t5,label_219_v96
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_226_v97
sw $t4,0($t0)
# Original instruction: li v98,0
li $t5,0
la $t0,label_225_v98
sw $t5,0($t0)
# Original instruction: slt v100,v97,v98
la $t5,label_226_v97
lw $t5,0($t5)
la $t4,label_225_v98
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_228_v100
sw $t3,0($t0)
# Original instruction: slt v101,v98,v97
la $t5,label_225_v98
lw $t5,0($t5)
la $t4,label_226_v97
lw $t4,0($t4)
slt $t3,$t5,$t4
la $t0,label_229_v101
sw $t3,0($t0)
# Original instruction: or v99,v100,v101
la $t5,label_228_v100
lw $t5,0($t5)
la $t4,label_229_v101
lw $t4,0($t4)
or $t3,$t5,$t4
la $t0,label_231_v99
sw $t3,0($t0)
# Original instruction: beqz v99,label_21_while_exit
la $t5,label_231_v99
lw $t5,0($t5)
beqz $t5,label_21_while_exit
# Original instruction: addiu v102,$fp,-36
addiu $t5,$fp,-36
la $t0,label_233_v102
sw $t5,0($t0)
# Original instruction: lw v103,0(v102)
la $t5,label_233_v102
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_235_v103
sw $t4,0($t0)
# Original instruction: addiu v104,v103,0
la $t5,label_235_v103
lw $t5,0($t5)
addiu $t4,$t5,0
la $t0,label_237_v104
sw $t4,0($t0)
# Original instruction: lw v105,0(v104)
la $t5,label_237_v104
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_239_v105
sw $t4,0($t0)
# Original instruction: add $a0,v105,$zero
la $t5,label_239_v105
lw $t5,0($t5)
add $a0,$t5,$zero
# Original instruction: li $v0,1
li $v0,1
# Original instruction: syscall
syscall
# Original instruction: add v106,$zero,$zero
add $t5,$zero,$zero
la $t0,label_240_v106
sw $t5,0($t0)
# Original instruction: addiu v107,$fp,-36
addiu $t5,$fp,-36
la $t0,label_242_v107
sw $t5,0($t0)
# Original instruction: lw v108,0(v107)
la $t5,label_242_v107
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_244_v108
sw $t4,0($t0)
# Original instruction: addiu v109,v108,4
la $t5,label_244_v108
lw $t5,0($t5)
addiu $t4,$t5,4
la $t0,label_246_v109
sw $t4,0($t0)
# Original instruction: lw v110,0(v109)
la $t5,label_246_v109
lw $t5,0($t5)
lw $t4,0($t5)
la $t0,label_249_v110
sw $t4,0($t0)
# Original instruction: addiu v111,$fp,-36
addiu $t5,$fp,-36
la $t0,label_250_v111
sw $t5,0($t0)
# Original instruction: sw v110,0(v111)
la $t5,label_249_v110
lw $t5,0($t5)
la $t4,label_250_v111
lw $t4,0($t4)
sw $t5,0($t4)
# Original instruction: j label_20_while_start
j label_20_while_start
label_21_while_exit:
label_19_return_main:
# Original instruction: addiu $sp,$sp,36
addiu $sp,$sp,36
# Original instruction: lw $ra,0($sp)
lw $ra,0($sp)
# Original instruction: lw $fp,4($sp)
lw $fp,4($sp)
# Original instruction: addiu $sp,$sp,8
addiu $sp,$sp,8
# Original instruction: li $v0,10
li $v0,10
# Original instruction: syscall
syscall

